...
const progressRows = (rawRows ?? []) as ProgressRow[];
  ...
  const entries = Object.entries(ROADMAPS).map(([slug, roadmap]) => {
    ...
    const doneStages = roadmap.stages.filter(
      (stage) => stage.resources.length > 0 && stage.resources.every((r) => completedResources.has(r.id))
    ).length;
    const total = roadmap.stages.length;
    return { slug, roadmap, done: doneStages, total, pct: total ? Math.round((doneStages / total) * 100) : 0 };
  });
        ...
        <div className="mt-10 space-y-4">
          ...
          {entries.length === 0 && (
            <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center dark:border-white/10 dark:bg-white/[0.02]">
              ...
              <Link href="/paths" className="mt-3 inline-block font-body text-sm text-amber hover:opacity-80">
                Browse paths →
              </Link>
            </div>
          )}
